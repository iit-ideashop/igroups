...
same as your username (or the first part of your e-mail address for non-IIT e-mails). Please change your password the first time you log the system.\n\n";
		...
		function getGroupsBySemester($semester) {
			$ipros = $this->db->igroupsQuery("SELECT iProjectID FROM PeopleProjectMap WHERE iPersonID=".$this->id." AND iSemesterID=$semester order by iProjectID");
			$returnArray = array();
			while($row = mysql_fetch_row($ipros))
		...
		{
			$query = $this->db->igroupsQuery("select iTaskID from TaskAssignments join Tasks on Tasks.iID=TaskAssignments.iTaskID where iPersonID={$this->id} and Tasks.iTeamID={$group->getID()}");
			$tasks = array();
