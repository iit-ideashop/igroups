...
if(!class_exists('Event'))
{
	class Event
	{
		var $id, $name, $desc, $eventdate, $group, $type, $semester, $valid;
		var $db;
		...
		function Event($id, $db)
		{
			...
			$this->id = $id;
			$this->db = $db;
			if($event = mysql_fetch_array($db->query("SELECT * FROM Events WHERE iID=$id")))
			{
		...
		function delete()
		{
			$this->db->query("DELETE FROM Events WHERE iID={$this->getID()}");
		}
		
		function updateDB()
		{
			$this->db->query("UPDATE Events SET sTitle='{$this->getNameDB()}', sDescription='{$this->getDescDB()}', dDate='{$this->getDateDB()}' WHERE iID={$this->id}");
		}
	}
	...
	function createEvent($name, $desc, $date, $group, $db)
	{
		...
		$db->query("INSERT INTO Events( sTitle, sDescription, dDate, iGroupID, iGroupType, iSemesterID, dCreateDate ) VALUES ( '".$namess->getDBString()."', '".$descss->getDBString()."', '$dbdate', ".$group->getID().", ".$group->getType().", ".$group->getSemester().", '".$currDate."' )");
		
		return new Event($db->insertID(), $db);
	}
	...
	function createIPROEvent($name, $desc, $date, $semester, $db)
	{
		...
		$db->query("INSERT INTO Events( sTitle, sDescription, dDate, iGroupID, iGroupType, iSemesterID, dCreateDate ) VALUES ( '".$namess->getDBString()."', '".$descss->getDBString()."', '$dbdate', 0, 0, ".$semester->getID().", '".$currDate."' )");
