...
if(!class_exists('Quota'))
{
	class Quota
	{
		...
		var $db;
		var $limitDefault = 2147483648; // 2 GiB (2^31 bytes)
		...
		function Quota($group, $db)
		{
			...
			if($quota = mysql_fetch_array($db->query($query)))
			{
			...
			else
			{
				...
				$db->query("INSERT INTO FileQuota( iLimit, iGroupID, iGroupType, iSemesterID ) VALUES ( ".$this->limitDefault.", ".$this->group.", ".$this->type.", ".$this->semester." )");
			}
		}
		...
		function sendWarning($warn)
		{
			if($this->type == 0)
			{
				$result = $this->db->query("SELECT sIITID from Projects WHERE iID=".$this->group);
				$array = mysql_fetch_array($result);
				$name = $array['sIITID'];
			}
			else
			{
				$result = $this->db->query("SELECT sName from Groups WHERE iID=".$this->group);
				$array = mysql_fetch_array($result);
				$name = $array['sName'];
			}
	...
	{
		$db->query("INSERT INTO FileQuota ( iGroupID, iGroupType, iSemesterID ) VALUES ( ".$group->getID().", ".$group->getType().", ".$group->getSemester()." )");
