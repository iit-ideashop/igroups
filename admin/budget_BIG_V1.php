...
<?php	
	$query = mysql_query("SELECT iProjectID, sum(bRequested) as requested, sum(bApproved) as approved, sum(bSpent) as spent, sum(bSpent)-sum(bApproved) as difference FROM Budgets WHERE iSemesterID={$currentSemester->getID()} GROUP by iProjectID ORDER by iProjectID");
	...
	if ($num_rows ==0) {
		...
		echo "<tr><th>IPRO Name</th><th>Requested</th><th>Approved</th><th>Actual/Forecast</th><th>Difference</th></tr>";
		...
		echo "<tr><td><a href=\"budget_details.php?iProjectID=$row[iProjectID]&iSemesterID=$this_semester&iproNum=$result[0]&iproName=$result[1]\"><strong>$result[0]:</strong> $result[1]</a></td><td>$row[requested]</td><td>$row[approved]</td><td>$row[spent]</td><td>$row[difference]</td></tr>";		
		}
		
		$semester_total = $db->igroupsQuery("SELECT sum(bRequested) as requested_total, sum(bApproved) as approved_total, sum(bSpent) as spent_total, sum(bSpent)-sum(bApproved) as difference_total  FROM Budgets WHERE iSemesterID={$currentSemester->getID()}");
