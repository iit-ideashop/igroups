...
if(!class_exists('ArPerson'))
{
	class ArPerson
	{
		...
		function removeFromGroup($groupID)
		{
			if(!is_numeric($groupID))
				return false;
			$this->db->query("delete from RatingsArchive where groupID=$groupID and (raterID={$this->id} or ratedID={$this->id})");
		...
		function addToGroup($groupID)
		{
			...
			if(!$this->isFaculty())
			{
				$this->db->query("insert into RatingsArchive (raterID, ratedID, groupID, listID) values ({$this->id}, {$this->id}, $groupID, {$group->getListID()})");
				$members = $group->getGroupStudents();
				foreach($members as $member)
				{
					$this->db->query("insert into RatingsArchive (raterID, ratedID, groupID, listID) values ({$this->id}, {$member->getID()}, $groupID, {$group->getListID()})");
					$this->db->query("insert into RatingsArchive (raterID, ratedID, groupID, listID) values ({$member->getID()}, {$this->id}, $groupID, {$group->getListID()})");
				}
			}
		...
		function getRatings()
		{
			$query = $this->db->query("select id from RatingsArchive where raterID={$this->id}");
			$ratings = array();
			while($row = mysql_fetch_row($query))
		...
		{
			$query = $this->db->query("select id from RatingsArchive where ratedID={$this->id} and groupID={$groupID}");
			$ratings = array();
			while ($row = mysql_fetch_row($query))
				$ratings[] = new Rating($row[0], $this->db);
			return $ratings;
		...
		{
			$query = $this->db->query("select COUNT(*) from RatingsArchive where raterID={$this->id}");
		...
		{
			$query = $this->db->query("select COUNT(*) from RatingsArchive where raterID={$this->id} and groupID={$groupID}");
			$row = mysql_fetch_row($query);
			return $row[0];
		}
		...
		{
			$this->usertype = 1;
			$this->db->query("delete from RatingsArchive where (ratedID={$this->id} or raterID={$this->id})");
		...
		{
			$query = $this->db->query("select rating from RatingsArchive where ratedID={$this->id} and groupID=$groupID");
		...
		{
			$query = $this->db->query("select AVG(rank) from RatingsArchive where ratedID={$this->id} and groupID=$groupID");
			$row = mysql_fetch_row($query);
			return $row[0];
		}
	...
	function createArPerson($email, $fname, $lname, $type, $db)
	{
		...
		$person = new ArPerson($db->insertID(), $db);			
